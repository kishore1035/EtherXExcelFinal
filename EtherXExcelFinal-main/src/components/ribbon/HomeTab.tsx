...
export function HomeTab({ isDarkMode }: HomeTabProps) {
  ...
  const handleApplyBorder = async (preset: BorderPreset) => {
    ...
    const setBorder = (row: number, col: number, side: 'top' | 'right' | 'bottom' | 'left', border: { style: string; color: string } | null) => {
      ...
      if (!newFormats[cellKey].borders) {
        newFormats[cellKey].borders = {};
      }
      
      if (border === null) {
        delete newFormats[cellKey].borders[side];
      } else {
        newFormats[cellKey].borders[side] = border;
      }
    };
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="flex flex-col gap-1">
            <Button 
              variant="ghost" 
              size="sm" 
              className={`h-4 px-2 text-xs ${buttonClass}`}
      ...
      <div className="flex flex-col gap-1 min-w-fit">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="px-2 py-1 hover:bg-gray-100 rounded text-gray-900"
          >
            Insert ▼
          </button>
          <button
