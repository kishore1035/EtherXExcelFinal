...
export function InsertTab({}: InsertTabProps) {
  ...
  return (
    <>
      <div className="flex items-center gap-2 sm:gap-4 px-2 sm:px-4 py-3 overflow-x-auto" style={{ transform: 'scale(0.75)', transformOrigin: 'left top', width: '133.33%' }}>
        {/* Illustrations Group */}
          ...
          <div className="flex items-center gap-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56 bg-white text-black">
                <DropdownMenuItem 
                  className="cursor-pointer hover:bg-gradient-to-r hover:from-[#FFFACD] hover:to-[#FFD700]"
                ...
                <DropdownMenuItem 
                  ...
                  onClick={() => {
                    if (selectedRange) {
                      removeValidationFromRange(`${getCellKey(selectedRange.startRow, selectedRange.startCol)}:${getCellKey(selectedRange.endRow, selectedRange.endCol)}`);
                      toast.success('Validation cleared from selected range');
                    } else if (selectedCell) {
                      removeValidationFromRange(getCellKey(selectedCell.row, selectedCell.col));
                      toast.success('Validation cleared from cell');
                    }
                  }}
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56 bg-white text-black">
                ...
                <DropdownMenuItem 
                  className="cursor-pointer hover:bg-gradient-to-r hover:from-[#FFFACD] hover:to-[#FFD700]"
                  onClick={() => {
                    // Quick highlight cells greater than
                ...
                <DropdownMenuItem 
                  ...
                  onClick={() => {
                    if (selectedRange) {
                      const range = `${getCellKey(selectedRange.startRow, selectedRange.startCol)}:${getCellKey(selectedRange.endRow, selectedRange.endCol)}`;
                      const rulesToDelete = conditionalFormattingRules.filter(rule => rule.range === range);
                      rulesToDelete.forEach(rule => deleteConditionalFormattingRule(rule.id));
                      toast.success('Formatting cleared from selected range');
                    }
                  }}
                  disabled={!selectedRange}
                >
          ...
          <div className="px-4 py-2 w-64">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                ...
                className={`px-3 py-1 rounded text-sm font-bold bg-black text-white hover:bg-gray-900 border border-transparent shadow`}
                disabled={!linkUrl.trim()}
                style={{ opacity: linkUrl.trim() ? 1 : 0.5, pointerEvents: 'auto', color: '#FFFFFF' }}
              >
                Insert Link
              </button>
            </div>
