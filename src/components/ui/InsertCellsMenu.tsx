...
export function InsertCellsMenu({ isOpen, onClose, isDarkMode, triggerRef }: InsertCellsMenuProps) {
  ...
  const menuItems = [
    { id: 'shift-right' as InsertAction, label: 'Shift Cells Right', icon: <ArrowRight className="w-5 h-5" style={{ color: '#3B82F6' }} /> },
    { id: 'shift-down' as InsertAction, label: 'Shift Cells Down', icon: <ArrowDown className="w-5 h-5" style={{ color: '#3B82F6' }} /> },
    { id: 'row-above' as InsertAction, label: 'Insert Row Above', icon: <RowsIcon className="w-5 h-5" style={{ color: '#16A34A' }} />, hasCount: true },
    { id: 'row-below' as InsertAction, label: 'Insert Row Below', icon: <RowsIcon className="w-5 h-5" style={{ color: '#16A34A' }} />, hasCount: true },
    { id: 'col-left' as InsertAction, label: 'Insert Column Left', icon: <Columns className="w-5 h-5" style={{ color: '#9333EA' }} />, hasCount: true },
    { id: 'col-right' as InsertAction, label: 'Insert Column Right', icon: <Columns className="w-5 h-5" style={{ color: '#9333EA' }} />, hasCount: true },
  ];
            ...
            {menuItems.map((item) => (
              <div key={item.id} className="flex items-center justify-between">
                <button
                  ...
                  style={!isDarkMode ? { color: '#000000 !important' } : undefined}
                >
                  {item.icon}
                  <span className="text-sm" style={{ color: !isDarkMode ? '#000000' : undefined }}>{item.label}</span>
                </button>
                {item.hasCount && (
                  <div className="flex items-center gap-1 ml-2">
                    <input
                      ...
                      style={{ color: !isDarkMode ? '#000000' : undefined }}
                    />
                  </div>
                )}
